...
export class CreateFirstLastList extends Component {
         ...
         render() {
           ...
           return <div className={styles.content}>
               ...
               <p>
                 Get started by curating a set of 10 songs for 99&#162;. You might have loads
                 of songs that you love, so your Last List can include as many sets as you
                 like and you can edit it whenever you want to.
               </p>
         ...
         renderList(list, listIndex) {
           return <div key={listIndex} className={styles.list}>
               {list.map((item, index) => <div key={index} className={styles.listRow}>
                   <Textfield label={index === 0 ? 'Artist Name' : null} placeholder="Artist Name" value={item.artistName} onChange={value => this.props.listsActions.setListItemArtist(listIndex, index, value)} />
                   <Textfield label={index === 0 ? 'Song Title' : null} placeholder="Song Title" value={item.songName} onChange={value => this.props.listsActions.setListItemSong(listIndex, index, value)} />
                 </div>)}
             </div>
         }
       }
